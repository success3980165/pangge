;(function($) {
	...
	CircleLoading.prototype.renderDOM = function(){
		...
		$.each(this.els , function(index, val) {
			...
			var base = $(this).find('.ed-circleloading-bg').attr('cx') === $(this).find('.ed-circleloading-tg').attr('cx') && $(this).find('.ed-circleloading-tg').attr('cy') === $(this).find('.ed-circleloading-bg').attr('cy') ? $(this).find('.ed-circleloading-bg').attr('cx'): null;
			...
			$(this).find('.ed-circleloading-bg').attr({
				'stroke': self.arrData[index].bgStroke.color,
				'stroke-width': self.arrData[index].bgStroke.width,
				'stroke-opacity': self.arrData[index].bgStroke.opacity
			});
			$(this).find('.ed-circleloading-tg').attr({
				'stroke': self.arrData[index].tgStroke.color,
				'stroke-width': self.arrData[index].tgStroke.width,
			...
			if (self.options.text) {
				var text ='<div class="ed-circleloading-text">'+ val +'<em>%</em></div>';
				$(this).prepend(text);
	...
	CircleLoading.prototype.bindDOM = function(){
		...
		$('.ed-circleloading-range').on('mousemove', change);   //mousemove事件和change事件，按需要使用。
	};
