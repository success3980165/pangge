...
Slider.prototype.bindDOM = function(){
	...
	var moveHandler = function(event){
		...
		//最小化改变DOM属性
		for(i; i < m; i++){
			lis[i] && (lis[i].style.webkitTransition = '-webkit-transform 0s ease-out'); //过度 0秒 取消卡顿
	...
	var endHandler = function(event){
		...
		if(endTime - self.startTime > 300){  //大于300毫秒 如果是慢操作
			if(self.offsetX >= boundary){  //进去上一页
				self.goIndex('-1');
			...
			//优化
			//快速移动也能使得翻页
			if(self.offsetX > 50){ //大于50偏移量 就划页
				self.goIndex('-1');
